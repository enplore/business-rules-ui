  ...
  ConditionsBuilder.prototype = {
    ...
    collectDataFromNode: function(element) {
      ...
      if(klass) {
        ...
        element.find("> .conditional, > .rule:visible:not(.results)").each(function() {
          out[klass].rules.push(_this.collectDataFromNode($(this)));
        });

        var results = element.find('> .rule.results:visible');
    ...
    buildConditional: function(ruleData, first) {
      ...
      addConditionLink.click(function(e) {
        ...
        div.find('>.results:first').before(_this.buildConditional(newField));
      });
      ...
      var colSel = getFieldSelect(this.fields, { name: 'foobar' }, 'Select an existing field');
      var colTxt = $('<input class="value textInput" type="text" placeholder="Any value">');

      var result = ruleData[kind].result;
      if (result.field) {
          colSel.find('option[value="' + result.field + '"]').attr('selected', true);
